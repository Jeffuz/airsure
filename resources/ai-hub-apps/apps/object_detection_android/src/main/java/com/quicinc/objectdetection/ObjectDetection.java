...
public class ObjectDetection  implements AutoCloseable {
    ...
    public ObjectDetection(Context context,
        ...
        android.util.Log.i("ObjectDetection", "Input Tensor Count: " + tfLiteInterpreter.getInputTensorCount());
        android.util.Log.i("ObjectDetection", "Output Tensor Count: " + tfLiteInterpreter.getOutputTensorCount());
        for (int i = 0; i < tfLiteInterpreter.getOutputTensorCount(); i++) {
            android.util.Log.i("ObjectDetection", "Output Tensor " + i + " shape: " + java.util.Arrays.toString(tfLiteInterpreter.getOutputTensor(i).shape()));
        }
        ...
        android.util.Log.i("ObjectDetection", "Input Tensor shape: " + java.util.Arrays.toString(inputShape));
        ...
        assert (inputShape[1] == 3 || inputShape[3] == 3); // Input tensor should have 3 channels (either at index 1 or 3)
        ...
        outputClassIs32bit = outputClassIdxType == DataType.INT32 || outputClassIdxType == DataType.FLOAT32;

        assert numBoxes == outputScoresShape[1];
        assert numBoxes == outputClassIdxShape[1];

        int inputHeight = (inputShape[1] == 3) ? inputShape[2] : inputShape[1];
        int inputWidth = (inputShape[1] == 3) ? inputShape[3] : inputShape[2];
        ...
        updatedBoxes = new float[numBoxes][4];
        scores = new float[numBoxes];
        classIdx = new int[numBoxes];
    ...
    public int getInputWidth() {
        return (inputShape[1] == 3) ? inputShape[3] : inputShape[2];
    }
    ...
    public int getInputHeight() {
        return (inputShape[1] == 3) ? inputShape[2] : inputShape[1];
    }
    ...
    public void predict(Bitmap image, int sensorOrientation, ArrayList<RectangleBox> BBlist) {
        ...
        if (inputShape[1] == 3) {
            // NCHW Format (YOLO standard) - Optimized using OpenCV split
            java.util.List<Mat> channels = new java.util.ArrayList<>(3);
            Core.split(scaledImage, channels);
            FloatBuffer floatBuffer = inputByteBuffer.asFloatBuffer();
            floatBuffer.rewind();
            float[] tempChannelArray = new float[inputHeight * inputWidth];
            for (Mat channel : channels) {
                channel.get(0, 0, tempChannelArray);
                floatBuffer.put(tempChannelArray);
                channel.release();
            }
        } else {
            // NHWC Format (Android standard)
        ...
        floatScoresBuf.get(this.scores);
        ...
        if (tfLiteInterpreter.getOutputTensor(2).dataType() == DataType.FLOAT32) {
            FloatBuffer floatBuffer = outputClassIdxBuffer.asFloatBuffer();
            float[] tempFloatClassIdx = new float[outputClassIdxShape[1]];
            floatBuffer.get(tempFloatClassIdx);
            for (int i = 0; i < tempFloatClassIdx.length; i++) {
                this.classIdx[i] = (int) tempFloatClassIdx[i];
            }
        } else if (outputClassIs32bit) {
            IntBuffer intBuffer = outputClassIdxBuffer.asIntBuffer();
            intBuffer.get(this.classIdx);
        } else {
            ...
            for (int i = 0; i < classIdxByte.length; ++i) {
                this.classIdx[i] = (int) classIdxByte[i];
            }
        }

        float scoreThreshold = 0.5f; // Increased for speed on emulator
        int maxDetections = 10;      // Reduced for speed on emulator

        for (int i = 0; i < numBoxes; i++) {
            if (this.scores[i] >= scoreThreshold) {
                float x0 = bboxes[i * 4];
        ...
        int[] result_indices = nmsProcessor.nmsScoreFilter(updatedBoxes, this.scores, maxDetections, 0.45f);
